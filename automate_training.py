...
story = ""
system_prompt = """ You are a model designed to continue narratives based on the previous text provided. Your task is to generate the next 1-2 paragraphs while 
                    carefully following the writing style, tone, entities, tags, and storyline established so far. 
                    Consider the context of the story, including characters, settings, and plot developments, to ensure that the new paragraphs flow seamlessly from the previous ones. 
                    If the current paragraph signals the end of a chapter, provide a fitting title, heading, and initial paragraphs for the next chapter, maintaining the same style, 
                    tone, and continuity.
                """
summary_so_far = ""
previous_paragraph = ""
...
for i in range(len(paras)):
    ...
    with open(f'{first_half_of_the_original_file}_training_data.jsonl', 'a') as f:
...
vFile = client.files.create(
    file=open(f"{first_half_of_the_original_file}_training_data.jsonl", "rb"),
    purpose="fine-tune"
)
